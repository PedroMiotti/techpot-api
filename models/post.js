    ...
    static async listPostsByUser(user_id, res) {
        ...
        await Sql.conectar(async (sql) => {
            try {
                ...
                //   [parseInt(user_id)]
                // );
                listPosts = await sql.query("SELECT p.post_id, p.post_body, p.post_body_html, p.post_like_count ,p.post_create_date, g.group_name, u.user_id,u.user_name\n" +
                    "FROM post p\n" +
                    "INNER JOIN user u \n" +
                    "ON p.user_id = u.user_id\n" +
                    "INNER JOIN group_pot g\n" +
                    "ON p.group_id = g.group_id\n" +
                    "WHERE g.group_id IN (SELECT group_id FROM group_membership WHERE user_id = ? ) \n" +
                    "ORDER BY p.post_id DESC", [parseInt(user_id)])
    ...
    static async listPostsByGroup(group_id, res) {
        ...
        await Sql.conectar(async (sql) => {
            try {
                ...
                post_list = await sql.query("SELECT p.post_id, p.post_body, p.post_body_html, p.post_create_date, p.post_like_count, g.group_name, u.user_id, u.user_name\n" +
                    "FROM post p\n" +
                    "INNER JOIN user u\n" +
                    "ON p.user_id = u.user_id\n" +
                    "INNER JOIN group_pot g\n" +
